...
def main():
    ...
    if pages_ignored_gt20:
        ...
        #print(f"→ Muestra: {len(pages_ignored_gt20)} documentos")
        print(f"→ Promedio de páginas: {avg_pages:.2f}")

        # Si hay muchas modas (empates), mostramos top frecuentes
        if len(modes_sorted) == 1:
            ...
            # además mostrar top N más frecuentes para contexto
            top = c.most_common(TOP_MODES_TO_SHOW)
            print(f"→ Top {TOP_MODES_TO_SHOW} páginas más frecuentes: {top}")

    #else:
    #    print("→ No hay documentos ignorados por >20 páginas (o todos fueron pages=0/error).")
